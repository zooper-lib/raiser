...
extension MiddlewareTestGenerators on Any {
  ...
  Generator<String?> get optionalBusName => any.choose([null, 'orders', 'payments', 'notifications', 'audit']);
  ...
  Generator<ConstructorInfo> get constructorInfo => any.choose([
    ...
    ConstructorInfo(
      ...
      parameters: [
        ...
        const ParameterInfo(name: 'ttl', type: 'Duration', isRequired: false, defaultValue: 'const Duration(minutes: 5)', isNamed: true),
      ],
    ),
  ]);
  ...
  Generator<MiddlewareInfo> get middlewareInfo => any.combine5(
    ...
    (className, priority, busName, sourceFile, constructor) =>
        MiddlewareInfo(className: className, priority: priority, busName: busName, sourceFile: sourceFile, constructor: constructor),
  );
}
...
void main() {
  group('MiddlewareInfo', () {
    ...
    Glados(any.middlewareInfo).test('MiddlewareInfo preserves all middleware attributes', (info) {
    ...
    Glados(any.middlewareInfo).test('toString contains all field values', (info) {
  ...
  group('Middleware Discovery Completeness', () {
    ...
    Glados(any.middlewareInfo).test('Property 5: each middleware is preserved for discovery', (middleware) {
    ...
    Glados(any.middlewareInfo).test('middleware registration info contains all required fields for discovery', (middleware) {
    ...
    Glados(any.intInRange(2, 5)).test('multiple middleware are all preserved', (count) {
      final middlewareList = List.generate(
        count,
        (i) => MiddlewareInfo(className: 'Middleware$i', priority: i, sourceFile: 'lib/middleware_$i.dart', constructor: const ConstructorInfo.noArgs()),
      );
  ...
  group('Middleware Priority Ordering', () {
    ...
    Glados(any.validPriority).test('Property 6: priority values are preserved for ordering', (priority) {
      final info = MiddlewareInfo(className: 'TestMiddleware', priority: priority, sourceFile: 'lib/test.dart', constructor: const ConstructorInfo.noArgs());
    ...
    Glados(any.intInRange(2, 10)).test('middleware can be sorted by priority descending', (count) {
      ...
      final sorted = List<MiddlewareInfo>.from(middlewareList)..sort((a, b) => b.priority.compareTo(a.priority));

      // Verify sorting is correct (descending order)
      for (var i = 0; i < sorted.length - 1; i++) {
        expect(sorted[i].priority, greaterThanOrEqualTo(sorted[i + 1].priority));
      }
    });

    /// Property test: Negative priorities are preserved and ordered correctly
    Glados(any.intInRange(-1000, -1)).test('negative priority values are preserved', (priority) {
      final info = MiddlewareInfo(className: 'TestMiddleware', priority: priority, sourceFile: 'lib/test.dart', constructor: const ConstructorInfo.noArgs());
    ...
    Glados(any.intInRange(1, 1000)).test('positive priority values are preserved', (priority) {
      final info = MiddlewareInfo(className: 'TestMiddleware', priority: priority, sourceFile: 'lib/test.dart', constructor: const ConstructorInfo.noArgs());
    ...
    test('middleware with equal priorities maintain stable order', () {
      final middlewareList = [
        MiddlewareInfo(className: 'FirstMiddleware', priority: 10, sourceFile: 'lib/first.dart', constructor: const ConstructorInfo.noArgs()),
        MiddlewareInfo(className: 'SecondMiddleware', priority: 10, sourceFile: 'lib/second.dart', constructor: const ConstructorInfo.noArgs()),
        MiddlewareInfo(className: 'ThirdMiddleware', priority: 10, sourceFile: 'lib/third.dart', constructor: const ConstructorInfo.noArgs()),
      ];
    ...
    test('mixed positive and negative priorities sort correctly', () {
      final middlewareList = [
        MiddlewareInfo(className: 'LowPriority', priority: -50, sourceFile: 'lib/low.dart', constructor: const ConstructorInfo.noArgs()),
        MiddlewareInfo(className: 'HighPriority', priority: 100, sourceFile: 'lib/high.dart', constructor: const ConstructorInfo.noArgs()),
        MiddlewareInfo(className: 'MediumPriority', priority: 0, sourceFile: 'lib/medium.dart', constructor: const ConstructorInfo.noArgs()),
      ];
  ...
  group('Middleware Validation Logic', () {
    ...
    test('error messages follow expected format for invalid middleware', () {
      ...
      const nonClassError = '@RaiserMiddleware can only be applied to classes. Found:';
      const abstractError = '@RaiserMiddleware cannot be applied to abstract classes.';
    ...
    Glados(any.optionalBusName).test('bus names are preserved correctly', (busName) {
